...
template <class Map>
inline void add_qty(Map &m, LimitOrderBook::ScaledPrice px,
...
template <class Map>
inline void sub_qty(Map &m, LimitOrderBook::ScaledPrice px,
                    LimitOrderBook::AggQty q) {
  auto it = m.find(px);
  if (it == m.end())
    return;
...
void LimitOrderBook::apply_add(char side, ScaledPrice px, AggQty qty) noexcept {
  if (qty == 0)
    return;
  if (side == 'B')
    add_qty(bids_, px, qty);
  else if (side == 'A')
    add_qty(asks_, px, qty);
}

void LimitOrderBook::apply_cancel(char side, ScaledPrice px,
                                  AggQty qty) noexcept {
  if (qty == 0)
    return;
  if (side == 'B')
    sub_qty(bids_, px, qty);
  else if (side == 'A')
    sub_qty(asks_, px, qty);
}

void LimitOrderBook::apply_fill(char side, ScaledPrice px,
                                AggQty filled_qty) noexcept {
...
bool LimitOrderBook::best_bid_scaled(ScaledPrice &px,
                                     AggQty &qty) const noexcept {
  if (bids_.empty())
    return false;
  auto it = bids_.begin();
  px = it->first;
...
bool LimitOrderBook::best_ask_scaled(ScaledPrice &px,
                                     AggQty &qty) const noexcept {
  if (asks_.empty())
    return false;
  auto it = asks_.begin();
...
LimitOrderBook::AggQty
LimitOrderBook::volume_at(char side, ScaledPrice px) const noexcept {
  if (side == 'B') {
